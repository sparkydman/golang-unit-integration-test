...
func database() {
	...
	dbConn = dao.MessageRepo.Initialize(dbDriver, username, password, port, host, database)
}
...
func seedOneMessage() (dto.Message, error) {
	msg := dto.Message{
		Title:     "the title",
		Body:      "the body",
		CreatedAt: time.Now(),
...
func seedMessages() ([]dto.Message, error) {
	msgs := []dto.Message{
		{
			Title:     "first title",
			Body:      "first body",
